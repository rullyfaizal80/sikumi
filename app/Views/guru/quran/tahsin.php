...
<head>
    ...
    <style>
        ...
        .btn-copy:hover { background-color: #17a2b8; color: white; border-color: #17a2b8; }
    </style>
</head>
<body class="p-4 bg-light">
    <div class="container-fluid">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                ...
                <p class="text-muted small">Kelola penilaian Tahsin siswa berdasarkan rentang pekan per bulan.</p>
            </div>
            <div>
        ...
        <div class="card shadow-sm border-0 mb-3">
            <div class="card-body py-3">
                <form action="<?= base_url('guru/quran/tahsin/'.$rombel['id']) ?>" method="GET" class="d-flex align-items-center">
                    ...
                    <select name="pekan" class="form-control form-control-sm mr-3 bg-info text-white font-weight-bold border-info" style="width: 90px;">
                        <?php for($p = 1; $p <= 5; $p++): ?>
                    ...
                    <table class="table table-bordered table-input mb-0 text-center" style="font-size: 13px;">
                        <thead>
                            <tr>
                                ...
                                <th colspan="4" class="text-info bg-light">KATEGORI TAHSIN (PEKAN <?= $pekan ?>)</th>
                            </tr>
                            <tr>
                                <th width="20%">
                                    TALQIN (Surat)<br>
                                    <button type="button" class="btn btn-outline-info btn-copy" onclick="copyToAll('input-talqin')">
                                        <i class="fas fa-copy"></i> Copy ke Bawah
                                    </button>
                                </th>
                                <th width="20%">
                                    RIYADHAH/TANAWUB (Halaman/Bab)<br>
                                    <button type="button" class="btn btn-outline-info btn-copy" onclick="copyToAll('input-riyadhah')">
                                        <i class="fas fa-copy"></i> Copy ke Bawah
                                    </button>
                                </th>
                        ...
                        <tbody>
                            <?php $no = 1; foreach ($daftarSiswa as $siswa): 
                                ...
                                $vTalqin   = isset($nilaiData[$sId]) ? $nilaiData[$sId]['tahsin_talqin'] : '';
                                $vRiyadhah = isset($nilaiData[$sId]) ? $nilaiData[$sId]['tahsin_riyadhah'] : '';
                                $vNilai    = isset($nilaiData[$sId]) ? $nilaiData[$sId]['tahsin_nilai'] : '';
                                $vCatatan  = isset($nilaiData[$sId]) ? $nilaiData[$sId]['tahsin_catatan'] : '';
                            ?>
                            <tr>
                                ...
                                <td>
                                    <input type="text" name="data[<?= $sId ?>][talqin]" class="form-control form-control-sm input-talqin" value="<?= esc($vTalqin) ?>" placeholder="Contoh: Al-Baqarah">
                                </td>
                                <td>
    ...
    <input type="text" name="data[<?= $sId ?>][nilai]" class="form-control form-control-sm font-weight-bold text-center" value="<?= esc($vNilai) ?>" oninput="this.value = this.value.replace(/[^0-9,]/g, '');">
                ...
                <div class="card-footer bg-white text-right py-3 border-top-0">
                    <button type="submit" class="btn btn-success font-weight-bold px-5 py-2 shadow-sm">
                        <i class="fas fa-save mr-2"></i> Simpan Data Tahsin Pekan <?= $pekan ?>
                    </button>
                </div>
            </div>
        </form>
    ...
    <script>
        function copyToAll(className) {
            // Ambil semua elemen input dengan class tertentu
            let inputs = document.getElementsByClassName(className);
            
            // Jika ada lebih dari satu baris siswa
            if (inputs.length > 0) {
                // Ambil nilai dari input pertama (Siswa No 1)
                ...
                // Terapkan ke semua input lain di kolom yang sama
