...
<body class="p-4 bg-light">
    ...
    <div class="container-fluid">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div>
                <h3 class="mb-0 text-success font-weight-bold"><i class="fas fa-campground mr-2"></i> <?= esc($title) ?></h3>
        ...
        <div class="card shadow-sm border-0">
            <form action="<?= base_url('guru/pramuka/save_nilai/'.$rombel['id']) ?>" method="POST">
                <?= csrf_field() ?>
                                ...
                                <tr class="sticky-header">
                                    ...
                                    <?php foreach($list_bulan as $bln): ?>
                                        <th>
                                            ...
                                            <?php else: ?>
                                                <br>
                                                <!-- TOMBOL COPY KE BAWAH -->
                                                <button type="button" class="btn btn-xs btn-primary mt-1 btn-copy-down" data-bulan="<?= $bln['angka'] ?>" title="Salin nilai siswa pertama ke seluruh siswa di bulan ini">
                                                    <i class="fas fa-arrow-down mr-1"></i> Copy
                                                </button>
                                            <?php endif; ?>
                                        </th>
                                    <?php endforeach; ?>
                ...
                <div class="card-footer bg-light py-3 d-flex justify-content-end">
                    <button type="submit" class="btn btn-success font-weight-bold px-4 shadow-sm">
                        <i class="fas fa-save mr-1"></i> Simpan Nilai Pramuka
                    </button>
                </div>
            </form>
        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // --- 2. LOGIKA FITUR COPY KE BAWAH ---
            const copyButtons = document.querySelectorAll('.btn-copy-down');
            copyButtons.forEach(btn => {
                btn.addEventListener('click', function() {
                    const bulanTarget = this.getAttribute('data-bulan');
                    
                    // Ambil semua input pada kolom bulan tersebut yang TIDAK readonly
                    const colInputs = document.querySelectorAll(`.nilai-input[data-bulan="${bulanTarget}"]:not([readonly])`);
                    
                    if (colInputs.length > 0) {
                        // Ambil nilai dari baris siswa pertama
                        const firstValue = colInputs[0].value;
                        
                        // Konfirmasi kecil agar guru tidak salah klik (Opsional, tapi aman)
                        if(confirm('Salin nilai "' + (firstValue || 'Kosong') + '" ke seluruh siswa di bulan ini?')) {
                            colInputs.forEach(input => {
                                input.value = firstValue;
                                
                                // Panggil event blur agar validasi float JS (koma) tetap berjalan
