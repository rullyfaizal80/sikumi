...
<head>
    ...
    .btn-cetak { background-color: #28a745; color: #fff; }
    ...
    @media print {
        ...
        .aksi-admin { display: none !important; } /* Menyembunyikan tombol saat print */
    }
</style>
</head>
<body>
...
<div class="rapor-container">
    ...
    <!-- AKSI ADMIN (Sembunyi saat cetak) -->
    <div class="aksi-admin" style="display: flex; justify-content: space-between; margin-bottom: 30px;">
        <a href="<?= base_url('admin/rapor-berjalan') ?>" class="btn-admin btn-kembali">&laquo; Kembali ke Filter</a>
        <button onclick="window.print()" class="btn-admin btn-cetak">Cetak Dokumen</button>
    </div>
   ...
   <!-- 1. PERKEMBANGAN AKADEMIK (SUMATIF) -->
    ...
    <!-- HAPUS table-layout: fixed; biarkan width 100% saja -->
    <table class="data-table" style="width: 100%;">
        <thead>
            <tr>
                <!-- Ubah lebar jadi 23% -->
                <th rowspan="2" style="width: 30%; background-color: #eaf3fa; color: #15202b; border-bottom: 3px solid #1976d2; text-align: center; vertical-align: middle;">Mata Pelajaran</th>
                
                <th colspan="<?= count($bulanAktif) ?>" style="background-color: #eaf3fa; color: #15202b; border-bottom: 1px solid #1976d2; text-align: center; vertical-align: middle;">Bulan Penilaian</th>
                
                <!-- Ubah lebar jadi 11% -->
                <th rowspan="2" style="width: 10%; background-color: #eaf3fa; color: #15202b; border-bottom: 3px solid #1976d2; text-align: center; vertical-align: middle;">Rata-rata<br>Semester</th>
            </tr>
            <tr>
                <?php foreach ($bulanAktif as $b): ?>
                    <!-- Ubah lebar jadi 11% -->
                    <th style="width: 10%; background-color: #eaf3fa; color: #15202b; border-bottom: 3px solid #1976d2; text-align: center; vertical-align: middle;"><?= $namaBulanIndo[$b] ?></th>
                <?php endforeach; ?>
            </tr>
        </thead>
        <tbody>
            ...
            <?php else: ?>
                <?php foreach ($matrixSumatif as $mapel): ?>
                    <tr>
                        ...
                        <?php foreach ($bulanAktif as $b): ?>
                            <td style="text-align: center;"><?= $fmt($mapel['nilai'][$b]) ?></td>
                        <?php endforeach; ?>
                        <td style="text-align: center; font-weight: bold; background-color: #e8f4fd; color: #0d47a1;">
    ...
    <table class="data-table" style="width: 100%;">
        <thead>
            <tr>
                <!-- Ubah lebar jadi 23% -->
                <th rowspan="2" style="width: 30%; background-color: #eaf3fa; color: #15202b; border-bottom: 3px solid #1976d2; text-align: center; vertical-align: middle;">Aspek Penilaian</th>
            ...
            <?php else: ?>
                <?php foreach ($matrixQuran as $aspek => $dataQuran): ?>
                    <tr>
                        ...
                        <?php foreach ($bulanAktif as $b): ?>
                            <td style="text-align: center;"><?= $fmt($dataQuran['nilai'][$b]) ?></td>
                        <?php endforeach; ?>
                        <!-- Style kolom rata-rata disesuaikan -->
                        <td style="text-align: center; font-weight: bold; background-color: #e8f4fd; color: #0d47a1;">
  ...
  <!-- 2. KEHADIRAN (ABSENSI) -->
    ...
    <table class="data-table" style="width: 100%;">
        <thead>
            <tr>
                <!-- Ubah lebar jadi 23% -->
                <th style="width: 30%; background-color: #eaf3fa; color: #15202b; border-bottom: 3px solid #1976d2; text-align: center; vertical-align: middle;">Keterangan</th>
                
                <?php foreach ($bulanAktif as $b): ?>
                    <!-- Ubah lebar jadi 11% -->
                    <th style="width: 10%; background-color: #eaf3fa; color: #15202b; border-bottom: 3px solid #1976d2; text-align: center; vertical-align: middle;">
                        <?= $namaBulanIndo[$b] ?>
                    </th>
                <?php endforeach; ?>
                
                <!-- Ubah lebar jadi 11% -->
                <th style="width: 10%; background-color: #eaf3fa; color: #15202b; border-bottom: 3px solid #1976d2; text-align: center; vertical-align: middle;">Total</th>
            </tr>
        </thead>
        <tbody>
            <?php
                $labelAbsen = [
                    ...
                    'T' => 'Frekuensi Keterlambatan',
                    'M' => 'Akumulasi Menit Terlambat'
                ];
                foreach ($labelAbsen as $kode => $label):
            ?>
            <tr>
                <!-- Hapus class, biarkan tag <th> di atas yang mengatur semua lebar ke bawah -->
                <td style="font-weight: 600;"><?= $label ?></td>
                
                <?php foreach ($bulanAktif as $b): ?>
                    <td style="text-align: center;"><?= esc($matrixAbsen[$kode][$b]) ?></td>
                <?php endforeach; ?>
                
                <!-- Kembalikan style warnanya agar serasi dengan kolom rata-rata tabel lain -->
                <td style="text-align: center; font-weight: bold; background-color: #e8f4fd; color: #0d47a1;">
                    <?= esc($totalAbsen[$kode]) ?>
                </td>
            </tr>
    ...
    <table class="data-table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <?php foreach ($bulanAktif as $b): ?>
                    <!-- ?: '-' sudah dihapus agar angka 0 tampil apa adanya -->
            ...
            <!-- BARIS BARU: Rincian Pelanggaran yang Digabung Kolomnya -->
            <tr style="background-color: #f9f9f9;">
                <td class="col-aspek" style="font-weight: bold; font-style: italic;">Rincian Pelanggaran:</td>
                <!-- Colspan menyesuaikan jumlah bulan + 1 kolom Total Kasus -->
